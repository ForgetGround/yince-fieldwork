...
侧边栏以「产品库」为入口。每款产品内查看匹配客户、关联政策规则和原始资料，规则不再单独展示。后端依据当前工作空间的产品规则计算匹配结果，区分基础条件匹配、信息待补充和已知条件不符，并逐条展示客户字段、版本与出处。

已录入税易贷、政采贷、智享贷、科企融、九融贷；原截图未注明名称的经营流水类产品单独保留，避免把其条件误套到其他产品。复杂分支与例外保留人工核查。匹配只辅助展业，不表示授信通过。访前快照、电话／微信记录与访后纪要沿用选定产品，规则升级不覆盖历史快照。
...
五分钟流程：管理员登录 → 产品库选择产品 → 核查匹配客户及政策依据 → 保存访前准备 → 登记沟通 → 确认访后纪要及任务 → 在团队权限范围内完成独立审查与跟进。
